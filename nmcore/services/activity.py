...
def record(guild_id:int, actor_id:int, actor_name:str, activity_type:str, title:str, details:str="", amount:int=0):
    def work():
        conn = db()
        cur = conn.cursor()
        cur.execute("""INSERT INTO live_activity
        (guild_id,actor_id,actor_name,activity_type,title,details,amount,created_at)
        VALUES (?,?,?,?,?,?,?,?)""", (
            int(guild_id), int(actor_id or 0), str(actor_name or "")[:120],
            str(activity_type), str(title)[:200], str(details)[:1200],
            int(amount or 0), int(time.time())
        ))
...
def log_event(guild_id:int, event_type:str, user_id:int=0, user_name:str="", channel_id:int=0, channel_name:str="", title:str="", details:str="", metadata=None):
    def work():
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?)""", (
            int(guild_id), str(event_type), int(user_id or 0), str(user_name)[:120],
            int(channel_id or 0), str(channel_name)[:120], str(title)[:200],
            str(details)[:2000], json.dumps(metadata or {}, ensure_ascii=False),
            int(time.time())
        ))
        conn.commit()
        conn.close()
