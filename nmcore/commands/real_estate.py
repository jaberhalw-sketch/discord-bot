...
def setup(bot):
    ...
    async def props(ctx):
        ...
        lines=[f"`{r['id']}` — **{r['display_name']}** | {r['price']:,} | rent {r['rent']:,}" for r in rows]
        await ctx.reply(embed=embed("🏘️ العقارات المتاحة","\n".join(lines) if lines else "لا يوجد عقارات متاحة","purple"))
