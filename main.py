...
LEGACY_DEFAULT_GUILD_ID = 1318663576210243616  # legacy only, NOT a bot lock
GUILD_ID = 0  # GLOBAL MODE: never restrict the bot to one guild
...
def nm_global_first_guild():
    ...
    try:
        gid = 0
        if "dashboard_get_active_guild_id" in globals():
            gid = int(dashboard_get_active_guild_id() or 0)
        elif "nm_active_guild_id" in globals():
            gid = int(nm_active_guild_id() or 0)
        if gid:
...
def create_default_guild_settings(guild):
    ...
    try:
        now = int(time.time())
        # GLOBAL MODE: every server starts clean and independent.
        # No guild gets the old hard-coded channel/category IDs automatically.
        commands_id = 0
        gambling_id = 0
        logs_id = 0
        setup_done = 0
...
async def require_commands_channel(ctx):
    if not ctx.guild:
        return False
    channel_id = get_effective_commands_channel_id(ctx.guild.id)
    # GLOBAL MODE: if this server has not configured a commands channel, allow commands anywhere.
    if not channel_id or (ctx.channel and ctx.channel.id == int(channel_id)):
        return True

    embed = discord.Embed(
        title="📍 الروم الغلط",
        description=f"استخدم أوامر اللفل والاقتصاد هنا: <#{int(channel_id)}>",
        color=COLOR_ORANGE
    )
...
async def require_gambling_channel(ctx):
    if not ctx.guild:
        return False
    channel_id = get_effective_gambling_channel_id(ctx.guild.id)
    # GLOBAL MODE: if this server has not configured a gambling channel, allow casino commands anywhere.
    if not channel_id or (ctx.channel and ctx.channel.id == int(channel_id)):
        return True

    embed = discord.Embed(
        title="🎰 الروم الغلط",
        description=f"أوامر القمار تشتغل فقط هنا: <#{int(channel_id)}>",
        color=COLOR_ORANGE
    )
    ...
    while not bot.is_closed():
        try:
            ...
            for guild in list(bot.guilds):
                channel_id = get_effective_commands_channel_id(guild.id) or ECONOMY_EXPLAIN_CHANNEL_ID
                channel = await get_channel_by_id(guild, channel_id) if channel_id else None
                if channel:
...
def is_dashboard_owner_user(user_id):
    ...
    try:
        ...
        if user_id in DASHBOARD_OWNER_USER_IDS or user_id in dashboard_dynamic_owner_user_ids():
            return True
        guild = nm_global_first_guild() if bot else None
...
<div class="layout">
  ...
  <aside class="sidebar">
    ...
    <nav class="navlist">
      ...
      <div class="navsection">Financial Control</div>
      <a class="navitem" href="/dashboard/money-tracker"><span class="navicon">💰</span><span>Money Tracker</span><span class="ownerlock">Owner</span></a>
      <a class="navitem" href="/dashboard/financial-audit-pro"><span class="navicon">🧾</span><span>Financial Audit Pro</span><span class="ownerlock">Owner</span></a>
      <a class="navitem" href="/dashboard/trace-status"><span class="navicon">🧬</span><span>Trace Status</span><span class="ownerlock">Owner</span></a>
      <div class="navsection">Real Estate</div>
      <a class="navitem" href="/dashboard/real-estate-monitor"><span class="navicon">🏘️</span><span>Real Estate Monitor</span><span class="ownerlock">Owner</span></a>
      <a class="navitem" href="/dashboard/real-estate-audit-pro"><span class="navicon">🏠</span><span>Real Estate Audit Pro</span><span class="ownerlock">Owner</span></a>
      {% endif %}
      <div class="navsection">Systems</div>
...
@bot.check
async def global_guild_guard(ctx):
    # GLOBAL MODE: allow commands in every guild where the bot is installed.
...
async def create_logs_command(ctx):
    ...
    if not guild:
        await ctx.send("❌ هذا الأمر يشتغل داخل السيرفر فقط.")
        return
...
async def create_or_repair_guild_log_channels(guild):
    ...
    for log_key, channel_name in LOG_CHANNEL_NAMES.items():
        ...
        if False:  # GLOBAL MODE: no legacy-guild special case
            try:
...
def nm_v4_normalize_settings_keys(settings):
    ...
    # Preserve owner's exact custom coin name.
    # Only use NM Coin if there is truly no saved value at all.
    coin = (
    ...
    settings["currency_name"] = str(coin)
    settings["economy_coin_name"] = str(coin)
    ...
    settings["bot_brand"] = str(brand)
    settings["brand_name"] = str(brand)
    settings["bot_name"] = str(brand)
...
def nm_fix_brand_coin_route():
    ...
    # This route now only syncs aliases. It does NOT force the coin name.
    # If you want the coin name changed, change it from dashboard customization.
...
@app.after_request
def nm_v4_coin_after_request_sync(response):
    try:
        if request.method == "POST" and request.path.startswith("/dashboard"):
            coin = request.form.get("coin_name") or request.form.get("currency_name") or request.form.get("economy_coin_name")
            brand = request.form.get("bot_brand") or request.form.get("brand_name") or request.form.get("bot_name")
            if coin or brand:
                gid = nm_v4_selected_gid_for_settings()
                settings = nm_v4_get_settings(gid)
                if coin and str(coin).strip():
                    settings["coin_name"] = str(coin).strip()
                    settings["currency_name"] = str(coin).strip()
                    settings["economy_coin_name"] = str(coin).strip()
                if brand and str(brand).strip():
                    settings["bot_brand"] = str(brand).strip()
                    settings["brand_name"] = str(brand).strip()
                    settings["bot_name"] = str(brand).strip()
                nm_v4_save_settings(gid, settings)
    except Exception as e:
        try:
            print(f"NM V4 coin after_request sync failed: {e}")
        except Exception:
            pass
    return response
...
# =====================================================================
# NM SYSTEM V4 FINAL POLISH SYNC
# هدفه: مصدر واحد للفلوس واسم العملة بين الداشبورد والديسكورد.
# - يقرأ الرصيد من PostgreSQL + SQLite القديم + guild_id=0
# - يكتب نفس الرصيد للسيرفر الحالي
# - أوامر الديسكورد والداشبورد واليوزر لوك أب تستخدم نفس الدالة
# - اسم العملة يقرأ من إعدادات السيرفر في PostgreSQL ويُحفظ بدون Reset
# =====================================================================

def nm_final_safe_int(v, default=0):
    try:
        return int(v or default)
    except Exception:
        return int(default)

def nm_final_selected_guild_id(source=None):
    try:
        if source is not None:
            if hasattr(source, "guild") and getattr(source, "guild", None):
                return int(source.guild.id)
...
def nm_final_sqlite_table_columns(cur, table):
    try:
        cur.execute(f"PRAGMA table_info({table})")
...
def nm_final_sqlite_table_exists(cur, table):
    try:
        cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name=?", (table,))
...
def nm_final_sqlite_paths():
    seen = []
    for p in [Path("/data/nm_system.db"), Path("nm_system.db")]:
        try:
            if p.exists() and p.stat().st_size > 0 and str(p) not in seen:
                seen.append(str(p))
                yield p
        except Exception:
            pass

def nm_final_pg_enabled():
    try:
        return bool(globals().get("NM_V4_POSTGRES_ENABLED") and globals().get("NM_DATABASE_URL"))
    except Exception:
        return False

def nm_final_pg_balance_sources(guild_id, user_id):
    out = []
    if not nm_final_pg_enabled():
        return out
    try:
        with nm_pg_conn() as conn:
            # selected guild
            ...
            if row:
                out.append(("pg_selected", int(row["balance"] or 0)))
            ...
            if row:
                out.append(("pg_legacy0", int(row["balance"] or 0)))

            # best anywhere for same user; this fixes old migration rows landing on wrong guild_id
            row = conn.execute(
                "SELECT guild_id, balance FROM economy WHERE user_id=%s ORDER BY balance DESC LIMIT 1",
                (int(user_id),)
            ).fetchone()
            if row:
                out.append((f"pg_any_{int(row['guild_id'] or 0)}", int(row["balance"] or 0)))
    except Exception as e:
        try: print(f"NM final PG balance source failed: {e}")
        except Exception: pass
    return out

def nm_final_sqlite_balance_sources(guild_id, user_id):
    out = []
    for path in nm_final_sqlite_paths():
        try:
            conn = sqlite3.connect(str(path))
            cur = conn.cursor()

            # V3 table
            if nm_final_sqlite_table_exists(cur, "guild_economy"):
                cols = nm_final_sqlite_table_columns(cur, "guild_economy")
                if "guild_id" in cols and "user_id" in cols and "balance" in cols:
                    for gid in [int(guild_id), 0, int(globals().get("GUILD_ID", 0) or 0)]:
                        cur.execute("SELECT balance FROM guild_economy WHERE guild_id=? AND user_id=?", (gid, int(user_id)))
                        row = cur.fetchone()
                        if row:
                            out.append((f"sqlite_guild_economy_{path.name}_{gid}", int(row[0] or 0)))
                    cur.execute("SELECT guild_id, balance FROM guild_economy WHERE user_id=? ORDER BY balance DESC LIMIT 1", (int(user_id),))
                    row = cur.fetchone()
                    if row:
                        out.append((f"sqlite_guild_economy_any_{row[0]}", int(row[1] or 0)))

            # Legacy economy table
            if nm_final_sqlite_table_exists(cur, "economy"):
                cols = nm_final_sqlite_table_columns(cur, "economy")
                if "guild_id" in cols and "user_id" in cols and "balance" in cols:
                    for gid in [int(guild_id), 0, int(globals().get("GUILD_ID", 0) or 0)]:
                        cur.execute("SELECT balance FROM economy WHERE guild_id=? AND user_id=?", (gid, int(user_id)))
                        row = cur.fetchone()
                        if row:
                            out.append((f"sqlite_economy_{path.name}_{gid}", int(row[0] or 0)))
                    cur.execute("SELECT guild_id, balance FROM economy WHERE user_id=? ORDER BY balance DESC LIMIT 1", (int(user_id),))
                    row = cur.fetchone()
                    if row:
                        out.append((f"sqlite_economy_any_{row[0]}", int(row[1] or 0)))
                elif "user_id" in cols and "balance" in cols:
                    cur.execute("SELECT balance FROM economy WHERE user_id=?", (int(user_id),))
                    row = cur.fetchone()
                    if row:
                        out.append((f"sqlite_legacy_economy_{path.name}", int(row[0] or 0)))

            conn.close()
        except Exception as e:
            try: print(f"NM final sqlite balance source failed {path}: {e}")
            except Exception: pass
    return out

def nm_final_write_pg_balance(guild_id, user_id, balance):
    if not nm_final_pg_enabled():
        return
    try:
        with nm_pg_conn() as conn:
            conn.execute("""
                ...
                ON CONFLICT (guild_id,user_id)
                DO UPDATE SET balance=GREATEST(economy.balance, EXCLUDED.balance), updated_at=NOW()
            """, (int(guild_id), int(user_id), int(balance or 0)))
            conn.commit()
    except Exception as e:
        try: print(f"NM final write PG balance failed: {e}")
        except Exception: pass

def nm_final_write_sqlite_balance(guild_id, user_id, balance):
    # Keep old dashboard pages from showing zero if they still read SQLite.
    for path in nm_final_sqlite_paths():
        try:
            conn = sqlite3.connect(str(path))
            cur = conn.cursor()

            # Ensure guild_economy exists in the old shape used by dashboard pages.
            cur.execute("""
                CREATE TABLE IF NOT EXISTS guild_economy (
                    guild_id INTEGER,
                    user_id INTEGER,
            ...
            """)
            cur.execute("""
                INSERT OR IGNORE INTO guild_economy (guild_id,user_id,balance)
                VALUES (?,?,0)
            """, (int(guild_id), int(user_id)))
            cur.execute("""
                UPDATE guild_economy
                SET balance=MAX(COALESCE(balance,0), ?)
                WHERE guild_id=? AND user_id=?
            """, (int(balance or 0), int(guild_id), int(user_id)))

            # Also mirror in economy if it has guild_id/user_id/balance.
            if nm_final_sqlite_table_exists(cur, "economy"):
                cols = nm_final_sqlite_table_columns(cur, "economy")
                if "guild_id" in cols and "user_id" in cols and "balance" in cols:
                    cur.execute("INSERT OR IGNORE INTO economy (guild_id,user_id,balance) VALUES (?,?,0)", (int(guild_id), int(user_id)))
                    cur.execute("UPDATE economy SET balance=MAX(COALESCE(balance,0), ?) WHERE guild_id=? AND user_id=?", (int(balance or 0), int(guild_id), int(user_id)))
                elif "user_id" in cols and "balance" in cols:
                    cur.execute("INSERT OR IGNORE INTO economy (user_id,balance) VALUES (?,0)", (int(user_id),))
                    cur.execute("UPDATE economy SET balance=MAX(COALESCE(balance,0), ?) WHERE user_id=?", (int(balance or 0), int(user_id)))

            conn.commit()
            conn.close()
        except Exception as e:
            try: print(f"NM final write sqlite balance failed {path}: {e}")
            except Exception: pass

def nm_final_get_balance(guild_id, user_id):
    guild_id = int(guild_id or 0)
    user_id = int(user_id or 0)
    if not user_id:
        return 0
    ...
    sources = []
    sources.extend(nm_final_pg_balance_sources(guild_id, user_id))
    sources.extend(nm_final_sqlite_balance_sources(guild_id, user_id))

    best = max([int(v or 0) for _, v in sources], default=0)

    # If any source has money, immediately sync all stores to selected guild.
    if best > 0:
        nm_final_write_pg_balance(guild_id, user_id, best)
        nm_final_write_sqlite_balance(guild_id, user_id, best)

    return int(best)

def nm_final_add_money(guild_id, user_id, amount, source_type="system", details="", actor_id=0):
    guild_id = int(guild_id or 0)
    user_id = int(user_id or 0)
    amount = int(amount or 0)

    current = nm_final_get_balance(guild_id, user_id)
        ...
        try:
            with nm_pg_conn() as conn:
                ...
                conn.execute("""
                    ...
                    (guild_id,user_id,amount,new_balance,source_type,details,actor_id,created_at)
                    VALUES (%s,%s,%s,%s,%s,%s,%s,%s)
                """, (guild_id, user_id, amount, new_balance, str(source_type), str(details), int(actor_id or 0), int(time.time())))
                conn.commit()
        except Exception as e:
            try: print(f"NM final add PG failed: {e}")
            except Exception: pass

    nm_final_write_sqlite_balance(guild_id, user_id, new_balance)
        ...
        try:
            with nm_pg_conn() as conn:
                rows = conn.execute("""
                    ...
                    LIMIT %s
                """, (guild_id, int(limit) * 4)).fetchall()
                for r in rows:
                    uid = int(r["user_id"])
                    merged[uid] = max(int(merged.get(uid, 0)), int(r["balance"] or 0))
        except Exception as e:
            try: print(f"NM final PG top failed: {e}")
            except Exception: pass
    ...
    for path in nm_final_sqlite_paths():
        try:
            ...
            for table in ["guild_economy", "economy"]:
                if not nm_final_sqlite_table_exists(cur, table):
                    continue
                cols = nm_final_sqlite_table_columns(cur, table)
                if "guild_id" in cols and "user_id" in cols and "balance" in cols:
                    cur.execute(f"""
                        SELECT user_id, MAX(balance) FROM {table}
                        WHERE guild_id IN (?,0)
                        GROUP BY user_id
                        ORDER BY MAX(balance) DESC
                        LIMIT ?
                    """, (guild_id, int(limit) * 4))
            ...
            try: print(f"NM final sqlite top failed {path}: {e}")
            except Exception: pass

    items = sorted(merged.items(), key=lambda kv: kv[1], reverse=True)[:int(limit)]
    # Sync top rows into canonical stores
    for uid, bal in items:
        if bal > 0:
            nm_final_write_pg_balance(guild_id, uid, bal)
...
def nm_final_settings_sources(guild_id):
    data = {}
    guild_id = int(guild_id or 0)
    ...
    if nm_final_pg_enabled():
        try:
            with nm_pg_conn() as conn:
                row = conn.execute("SELECT settings FROM guild_settings WHERE guild_id=%s", (guild_id,)).fetchone()
                if row:
                    data.update(nm_pg_unjson(row["settings"]))
        except Exception as e:
            try: print(f"NM final settings PG failed: {e}")
            except Exception: pass

    # SQLite guild_settings fallback
    for path in nm_final_sqlite_paths():
        try:
            conn = sqlite3.connect(str(path))
            conn.row_factory = sqlite3.Row
            cur = conn.cursor()
            if nm_final_sqlite_table_exists(cur, "guild_settings"):
                cols = nm_final_sqlite_table_columns(cur, "guild_settings")
                if "guild_id" in cols:
                    cur.execute("SELECT * FROM guild_settings WHERE guild_id=? LIMIT 1", (guild_id,))
                    row = cur.fetchone()
                    if row:
                        for k in row.keys():
                            if k not in ("guild_id","id","created_at","updated_at") and row[k] not in (None, ""):
                                data.setdefault(k, row[k])
            conn.close()
        except Exception:
            pass

    # loaded JSON/global fallback
    try:
        if "dashboard_settings" in globals() and isinstance(dashboard_settings, dict):
...
def nm_final_normalize_settings(settings):
    settings = dict(settings or {})
    coin = (
        settings.get("coin_name")
        or settings.get("currency_name")
...
def nm_final_get_settings(guild_id=None):
    return nm_final_normalize_settings(nm_final_settings_sources(guild_id or nm_final_selected_guild_id()))

def nm_final_save_settings(guild_id=None, settings=None):
    gid = int(guild_id or nm_final_selected_guild_id())
    settings = nm_final_normalize_settings(settings or {})
    ...
    if nm_final_pg_enabled():
        try:
            ...
            try: print(f"NM final save settings PG failed: {e}")
            except Exception: pass

    # Global/json runtime mirror so old customization center displays the right value immediately.
    try:
        if "dashboard_settings" in globals() and isinstance(dashboard_settings, dict):
            dashboard_settings.update(settings)
            if "save_dashboard_settings" in globals():
...
def nm_final_sync_dashboard_forms(response):
    try:
        ...
        try: print(f"NM final after_request form sync failed: {e}")
        except Exception: pass
    return response

@app.route("/dashboard/final-sync")
def nm_final_sync_route():
    gid = nm_final_selected_guild_id()
    moved = 0
    for uid, bal in nm_final_top_balances(gid, 10000):
        if bal > 0:
            nm_final_write_pg_balance(gid, uid, bal)
            nm_final_write_sqlite_balance(gid, uid, bal)
            moved += 1
    settings = nm_final_get_settings(gid)
    nm_final_save_settings(gid, settings)
    return f"""
    <div style="font-family:Arial;background:#0b1020;color:white;min-height:100vh;padding:40px">
      <h1>Final Sync Done</h1>
      <p>Guild: <b>{int(gid)}</b></p>
      <p>Synced balances: <b>{int(moved)}</b></p>
      <p>Coin now: <b>{dash_escape(nm_coin_name(gid), 100)}</b></p>
      <p>Now test <code>!رصيدي</code> and refresh User Lookup.</p>
      <p><a style="color:#8b5cf6" href="/dashboard?guild_id={int(gid)}">Back to Dashboard</a></p>
    </div>
    """
...
# =====================================================================
# NM V5 ADMIN ACCESS PRO+
# Upgrades Admin Access:
# - Better people-card UI
# - Stats cards
# - Live save
# - Audit history
...
def nm_v5_admin_audit_path():
    try:
        p = Path("/data")
        p.mkdir(parents=True, exist_ok=True)
        return p / "admin_access_audit_v5.json"
    except Exception:
        return Path("admin_access_audit_v5.json")

def nm_v5_admin_audit_load():
    try:
        p = nm_v5_admin_audit_path()
        if p.exists() and p.stat().st_size > 0:
            data = json.loads(p.read_text(encoding="utf-8") or "[]")
            return data if isinstance(data, list) else []
    except Exception:
...
def nm_v5_admin_audit_save(data):
    try:
        p = nm_v5_admin_audit_path()
        p.write_text(json.dumps(data[-300:] if isinstance(data, list) else [], ensure_ascii=False, indent=2), encoding="utf-8")
        return True
    except Exception:
        return False

def nm_v5_admin_actor_name():
    try:
        user = session.get("discord_user") or {}
        return str(user.get("username") or user.get("global_name") or user.get("id") or "Dashboard")
    except Exception:
        return "Dashboard"

def nm_v5_admin_audit(gid, action, target="", details=""):
    try:
        data = nm_v5_admin_audit_load()
        data.append({
            "guild_id": int(gid),
            "action": str(action),
            "target": str(target),
            "details": str(details),
            "actor": nm_v5_admin_actor_name(),
            "time": int(time.time()),
        })
        nm_v5_admin_audit_save(data)
    except Exception:
        pass

def nm_v5_admin_count_access(settings, members=None):
    owner_count = 0
    admin_count = 0
    try:
        if members:
            ...
            owner_count = len(settings.get("owner_user_ids", [])) + len(settings.get("owner_role_ids", []))
            admin_count = len(settings.get("admin_user_ids", [])) + len(settings.get("admin_role_ids", []))
    except Exception:
        pass
    return owner_count, admin_count

def nm_v5_admin_pro_save_user(gid, user_id, access):
    settings = nm_v5_admin_get(gid)
    owner_users = set(nm_v5_admin_clean_ids(settings.get("owner_user_ids", [])))
    admin_users = set(nm_v5_admin_clean_ids(settings.get("admin_user_ids", [])))

    uid = int(user_id)

    # last owner protection
    if access == "remove" and uid in owner_users and len(owner_users) <= 1 and not settings.get("owner_role_ids"):
        return False, "Cannot remove the last Owner. Add another Owner first."

    owner_users.discard(uid)
    ...
    settings["owner_user_ids"] = sorted(owner_users)
    settings["admin_user_ids"] = sorted(admin_users)
    settings["updated_at"] = int(time.time())
    settings["updated_by"] = nm_v5_admin_actor_name()
    nm_v5_admin_set(gid, settings)

    nm_v5_admin_audit(gid, f"set_user_{access}", target=str(uid), details=f"user_id={uid} access={access}")
    return True, "Saved"

@app.route("/dashboard/admin-access-user", methods=["POST"])
def nm_v5_admin_access_user_api():
    denied = dashboard_require_owner()
    if denied:
        return denied
    gid = nm_v5_admin_gid()
    uid = request.form.get("user_id") or request.json.get("user_id") if request.is_json else request.form.get("user_id")
    access = request.form.get("access") or request.json.get("access") if request.is_json else request.form.get("access")
    try:
        ok, msg = nm_v5_admin_pro_save_user(gid, int(uid), str(access))
    except Exception as e:
        ok, msg = False, str(e)
    return jsonify({"ok": bool(ok), "message": msg})

@app.route("/dashboard/admin-access-export")
def nm_v5_admin_access_export():
    denied = dashboard_require_owner()
    if denied:
        return denied
    gid = nm_v5_admin_gid()
    settings = nm_v5_admin_get(gid)
    audit = [x for x in nm_v5_admin_audit_load() if int(x.get("guild_id", 0)) == int(gid)][-50:]
    payload = {
        "guild_id": int(gid),
        "settings": settings,
        "audit_tail": audit,
...
def nm_v5_admin_audit_html(gid, limit=8):
    rows = [x for x in nm_v5_admin_audit_load() if int(x.get("guild_id", 0)) == int(gid)][-limit:]
    rows = list(reversed(rows))
    if not rows:
        return "<div class='empty'>No Admin Access changes yet.</div>"
    html = []
    for r in rows:
        try:
            t = time.strftime("%Y-%m-%d %H:%M", time.localtime(int(r.get("time", 0) or 0)))
        except Exception:
            t = "Unknown"
        html.append(f"""
        <div class="audit-row">
          <div><b>{dash_escape(r.get('action',''), 80)}</b><div class="muted small">{dash_escape(r.get('details',''), 180)}</div></div>
          <div class="muted small">{dash_escape(r.get('actor',''), 80)}<br>{t}</div>
        </div>
        """)
    return "".join(html)

def nm_v5_admin_people_page_pro():
    if not dashboard_session_is_private_owner(session.get("discord_user") or {}):
        denied = dashboard_require_owner()
        if denied:
            return denied
    ...
    clean_members = sorted(clean_members, key=sort_key)
    owner_count, admin_count = nm_v5_admin_count_access(settings, clean_members)
    ...
    for m in clean_members:
        ...
        cards.append(nm_v5_admin_people_card(m, access, gid))
    if not cards:
        cards.append("<div class='empty'>No members loaded. اضغط Refresh Members أو تأكد إن Server Members Intent شغال.</div>")
    ...
    owner_users_value = ", ".join(str(x) for x in settings.get("owner_user_ids", []))
    admin_users_value = ", ".join(str(x) for x in settings.get("admin_user_ids", []))
    updated_at = int(settings.get("updated_at") or 0)
    updated_txt = time.strftime("%Y-%m-%d %H:%M", time.localtime(updated_at)) if updated_at else "Never"

    body = f"""
    <style>
      .pro-hero {{
        background:linear-gradient(135deg,#0b1224,#1d1550);
        border:1px solid #26345c; border-radius:26px; padding:22px; margin-bottom:18px;
        display:flex; align-items:center; justify-content:space-between; gap:14px; flex-wrap:wrap;
      }}
      .pro-stats {{ display:flex; gap:12px; flex-wrap:wrap; }}
      .pro-stat {{ background:#081126; border:1px solid #26345c; border-radius:18px; padding:14px 18px; min-width:130px; }}
      .pro-stat b {{ font-size:28px; }}
      .admin-topbar {{
        display:flex; gap:12px; align-items:center; justify-content:space-between; flex-wrap:wrap;
        background:#0b1224; border:1px solid #1e293b; border-radius:22px; padding:18px; margin-bottom:18px;
      }}
      ...
      .access-badge.none {{ background:#111827; color:#94a3b8; }}
      .member-actions {{ display:flex; gap:8px; flex-wrap:wrap; justify-content:flex-end; }}
      .small-btn {{ border:0; border-radius:12px; padding:10px 12px; font-weight:1000; cursor:pointer; color:white; }}
      .owner-btn {{ background:#b45309; }}
      .admin-btn {{ background:#2563eb; }}
      .remove-btn {{ background:#374151; }}
      .advanced-box {{ display:none; margin-top:16px; }}
      ...
      select[multiple] {{ min-height:160px; }}
      .audit-row {{
        display:flex; justify-content:space-between; gap:12px; background:#0b1224; border:1px solid #1e293b;
    ...
    <div class="pro-hero">
      <div>
        <h2>🔐 Admin Access Pro+</h2>
        <p class="muted">تحكم مباشر بالأشخاص بصورهم وأسمائهم. كل تغيير ينحفظ لايف وما يضيع بعد Redeploy.</p>
        <p><span class="pill ok">Saved in /data/admin_access_v5.json</span> <span id="saveState" class="pill">Ready</span></p>
      </div>
      <div class="pro-stats">
        <div class="pro-stat"><b>{owner_count}</b><br><span class="muted">Owners</span></div>
        <div class="pro-stat"><b>{admin_count}</b><br><span class="muted">Admins</span></div>
        <div class="pro-stat"><b>{len(clean_members)}</b><br><span class="muted">Members</span></div>
        <div class="pro-stat"><b>{updated_txt}</b><br><span class="muted">Last Save</span></div>
      </div>
    </div>
    ...
    <div class="admin-topbar">
      ...
      <a class="btn" href="/dashboard/admin-access?refresh=1&guild_id={int(gid)}">🔄 Refresh</a>
      <a class="btn" href="/dashboard/admin-access-export?guild_id={int(gid)}">📦 Export</a>
      <button class="btn" type="button" onclick="toggleAdvanced()">⚙️ Advanced Roles</button>
    </div>
    ...
    <div class="card">
      <h3>👥 Members</h3>
      <div id="memberList" class="member-list">{''.join(cards)}</div>
    </div>

    <div class="card">
      <h3>🧾 Admin Access Audit</h3>
      {nm_v5_admin_audit_html(gid)}
    </div>
    ...
    <script>
      ...
      async function saveAccess() {{
        ...
        const fd = new FormData(form);
        setSaveState('Saving...');
        const res = await fetch('/dashboard/admin-access-live', {{ method:'POST', body:fd, headers:{{'X-Requested-With':'fetch'}} }});
        setSaveState(res.ok ? 'Saved ✓' : 'Save failed');
        setTimeout(() => setSaveState('Ready'), 1600);
        return res.ok;
      }}
      async function setUserAccess(userId, access) {{
        ...
        if (access === 'remove' && owners.includes(userId) && owners.length <= 1) {{
          alert('ما تقدر تشيل آخر Owner. أضف Owner ثاني أول.');
          return;
        }}
    ...
    """
    return render_dashboard_page("Admin Access Pro+", body)

try:
    app.view_functions["dashboard_admin_access_page"] = nm_v5_admin_people_page_pro
except Exception as e:
    try: print(f"NM V5 Admin Access Pro+ override skipped: {e}")
    except Exception: pass

def nm_v5_admin_pro_boot():
    try:
